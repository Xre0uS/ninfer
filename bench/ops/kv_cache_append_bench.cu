...
std::size_t full_code_bytes(const FullGeometry& geometry, const ops::D256KVVectorProfile& profile,
                            std::int32_t padded) {
    return static_cast<std::size_t>(profile.code_leading_extent) * geometry.kv_heads * padded *
           dtype_size(profile.code_dtype);
}

std::size_t full_scale_bytes(const FullGeometry& geometry, const ops::D256KVVectorProfile& profile,
                             std::int32_t padded) {
    return static_cast<std::size_t>(profile.scale_leading_extent) * geometry.kv_heads * padded *
           dtype_size(profile.scale_dtype);
}
...
PagedKVLayerView make_full_view(DeviceBuffer& k, DeviceBuffer& v, DeviceBuffer& k_scale,
    ...
    return {
        .k_pages =
            Tensor(k.p, profile.key.code_dtype,
                   {profile.key.code_leading_extent, kPagedKVPageSize, geometry.kv_heads, pages}),
        .v_pages =
            Tensor(v.p, profile.value.code_dtype,
                   {profile.value.code_leading_extent, kPagedKVPageSize, geometry.kv_heads, pages}),
        .k_scale_pages = profile.key.scaled() ? Tensor(k_scale.p, profile.key.scale_dtype,
                                                       {profile.key.scale_leading_extent,
                                                        kPagedKVPageSize, geometry.kv_heads, pages})
                                              : Tensor(),
...
public:
    FullCase(FullGeometry geometry, KvCacheStorage storage, std::int32_t tokens,
             std::int32_t context)
        : geometry_(geometry), storage_(storage), profile_(ops::d256_kv_cache_profile(storage)),
          tokens_(tokens), capacity_(context + tokens), padded_(align_context(capacity_)),
          ...
          cache_k_(bench::make_zeros(full_code_bytes(geometry, profile_.key, padded_))),
          cache_v_(bench::make_zeros(full_code_bytes(geometry, profile_.value, padded_))),
          cache_k_scale_(bench::make_zeros(profile_.key.scaled()
                                               ? full_scale_bytes(geometry, profile_.key, padded_)
                                               : std::size_t{1})),
          cache_v_scale_(bench::make_zeros(profile_.value.scaled()
                                               ? full_scale_bytes(geometry, profile_.value, padded_)
                                               : std::size_t{1})),
...
double full_vector_bytes(const ops::D256KVVectorProfile& profile) {
...
double full_logical_cache_bytes(const FullGeometry& geometry, std::int32_t tokens) {
    return static_cast<double>(ops::D256KVCacheProfile::logical_bytes_per_token_head()) *
           geometry.kv_heads * tokens;
}
...
double full_physical_cache_bytes(const FullGeometry& geometry, KvCacheStorage storage,
    ...
    return geometry.kv_heads * tokens *
           (full_vector_bytes(profile.key) + full_vector_bytes(profile.value));
}
            ...
            for (const FullGeometry& geometry : geometries) {
                for (const KvCacheStorage storage : storages) {
                    for (const std::int32_t tokens : options.tokens) {
                        const auto profile              = ops::d256_kv_cache_profile(storage);
                        const double key_vector_bytes   = full_vector_bytes(profile.key);
                        const double value_vector_bytes = full_vector_bytes(profile.value);
