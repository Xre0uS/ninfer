...
constexpr double kDenseNvfp4TcTflops     = 838.0;
...
std::size_t code_plane_bytes(const Geometry& geometry, const ops::D256KVVectorProfile& profile,
                             std::int32_t physical_pages) {
    return static_cast<std::size_t>(profile.code_leading_extent) * geometry.kv_heads *
           kPagedKVPageSize * physical_pages * dtype_size(profile.code_dtype);
}

std::size_t scale_plane_bytes(const Geometry& geometry, const ops::D256KVVectorProfile& profile,
                              std::int32_t physical_pages) {
    return static_cast<std::size_t>(profile.scale_leading_extent) * geometry.kv_heads *
           kPagedKVPageSize * physical_pages * dtype_size(profile.scale_dtype);
}
...
PagedKVLayerView make_cache_view(DeviceBuffer& k, DeviceBuffer& v, DeviceBuffer& k_scale,
    ...
    return {
        .k_pages = Tensor(
            k.p, profile.key.code_dtype,
            {profile.key.code_leading_extent, kPagedKVPageSize, geometry.kv_heads, physical_pages}),
        .v_pages       = Tensor(v.p, profile.value.code_dtype,
                                {profile.value.code_leading_extent, kPagedKVPageSize, geometry.kv_heads,
                                 physical_pages}),
        .k_scale_pages = profile.key.scaled()
                             ? Tensor(k_scale.p, profile.key.scale_dtype,
                                      {profile.key.scale_leading_extent, kPagedKVPageSize,
                                       geometry.kv_heads, physical_pages})
                             : Tensor(),
        .v_scale_pages = profile.value.scaled()
                             ? Tensor(v_scale.p, profile.value.scale_dtype,
                                      {profile.value.scale_leading_extent, kPagedKVPageSize,
                                       geometry.kv_heads, physical_pages})
                             : Tensor(),
...
public:
    Case(Geometry geometry, KvCacheStorage storage, std::int32_t tokens,
        ...
        : profile_(ops::d256_kv_cache_profile(storage)),
          ...
          cache_k_(bench::make_zeros(code_plane_bytes(geometry, profile_.key, physical_pages_))),
          cache_v_(bench::make_zeros(code_plane_bytes(geometry, profile_.value, physical_pages_))),
          cache_k_scale_(bench::make_zeros(
              profile_.key.scaled() ? scale_plane_bytes(geometry, profile_.key, physical_pages_)
                                    : std::size_t{1})),
          cache_v_scale_(bench::make_zeros(
              profile_.value.scaled() ? scale_plane_bytes(geometry, profile_.value, physical_pages_)
                                      : std::size_t{1})),
...
double cache_vector_bytes(const ops::D256KVVectorProfile& profile) {
    return static_cast<double>(profile.physical_bytes());
}
...
double modeled_bytes(Entry entry, const Geometry& geometry, KvCacheStorage storage,
    ...
    const double key_bytes =
        physical_cache ? cache_vector_bytes(profile.key) : profile.key.logical_bytes();
    const double value_bytes =
        physical_cache ? cache_vector_bytes(profile.value) : profile.value.logical_bytes();
...
double cache_footprint_bytes(const Geometry& geometry, KvCacheStorage storage,
    ...
    const auto profile = ops::d256_kv_cache_profile(storage);
    const double key_bytes =
        physical ? cache_vector_bytes(profile.key) : profile.key.logical_bytes();
    const double value_bytes =
        physical ? cache_vector_bytes(profile.value) : profile.value.logical_bytes();
...
void report(const Result& result) {
    ...
    const double qk_peak = result.storage == KvCacheStorage::Nvfp4Group16 ? kDenseNvfp4TcTflops
                           : (result.storage == KvCacheStorage::Fp8E4M3Row256 ||
                              result.storage == KvCacheStorage::K8V4)
                               ? kDenseFp8TcTflops
...
void write_csv(const Options& options, const std::vector<Result>& results) {
    ...
    output << "entry,geometry,kv_dtype,mapping,execution,cache,B,W,row_contexts,valid_columns,"
              ...
              "qk_full_op_tflops,pv_full_op_tflops,mixed_tc_floor_us,mixed_tc_roofline_pct,"
...
std::vector<KvCacheStorage> selected_storages(KvChoice choice) {
    ...
    if (choice == KvChoice::K8V4) { return {KvCacheStorage::K8V4}; }
                                    ...
                                    for (const CacheState cache :
                                        ...
                                        Result result{
                                            ...
                                            cache_footprint_bytes(geometry, storage, rows.contexts,
                                                                  rows.valid_columns, false),
                                            cache_vector_bytes(profile.key),
                                            cache_vector_bytes(profile.value),
                                            cache_footprint_bytes(geometry, storage, rows.contexts,
                                                                  rows.valid_columns, true),
