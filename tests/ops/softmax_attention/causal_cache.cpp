...
int verify_nvfp4_mma_probe() {
    std::vector<std::uint8_t> a(16 * 128);
    std::vector<std::uint8_t> b(8 * 128);
    std::vector<std::uint8_t> as(16 * 16);
    std::vector<std::uint8_t> bs(8 * 16);
    constexpr std::array<float, 8> e2{0.0f, 0.5f, 1.0f, 1.5f, 2.0f, 3.0f, 4.0f, 6.0f};
    constexpr std::array<float, 4> scales{0.5f, 1.0f, 2.0f, 4.0f};
    const auto decode = [&](std::uint8_t code) {
        const float value = e2[static_cast<std::size_t>(code & 7U)];
        return (code & 8U) == 0 ? value : -value;
    };
    for (int row = 0; row < 16; ++row) {
        for (int group = 0; group < 16; ++group) {
            as[static_cast<std::size_t>(row * 16 + group)] =
                static_cast<std::uint8_t>(0x30 + 8 * ((row + group) & 3));
            for (int pair = 0; pair < 8; ++pair) {
                std::uint8_t low  = static_cast<std::uint8_t>((row + group + 2 * pair) & 7);
                std::uint8_t high = static_cast<std::uint8_t>((3 * row + group + 2 * pair + 1) & 7);
                if (((row + pair) & 1) != 0) low |= 8U;
                if (((group + pair) & 1) != 0) high |= 8U;
                a[static_cast<std::size_t>(row * 128 + group * 8 + pair)] =
    ...
    for (int row = 0; row < 8; ++row) {
        for (int group = 0; group < 16; ++group) {
            bs[static_cast<std::size_t>(row * 16 + group)] =
                static_cast<std::uint8_t>(0x30 + 8 * ((2 * row + group) & 3));
            for (int pair = 0; pair < 8; ++pair) {
                std::uint8_t low  = static_cast<std::uint8_t>((2 * row + group + pair) & 7);
                std::uint8_t high = static_cast<std::uint8_t>((row + 3 * group + pair + 1) & 7);
                if (((row + group + pair) & 1) != 0) low |= 8U;
                if (((row + pair) & 1) != 0) high |= 8U;
                b[static_cast<std::size_t>(row * 128 + group * 8 + pair)] =
                    static_cast<std::uint8_t>(low | (high << 4));
            }
        }
    }
    DeviceBuffer da  = to_device(a);
    DeviceBuffer db  = to_device(b);
    DeviceBuffer das = to_device(as);
    DeviceBuffer dbs = to_device(bs);
    DeviceBuffer dout(16 * 8 * sizeof(float));
    launch_nvfp4_mma_probe(
        static_cast<const std::uint8_t*>(da.p), static_cast<const std::uint8_t*>(db.p),
        static_cast<const std::uint8_t*>(das.p), static_cast<const std::uint8_t*>(dbs.p),
        static_cast<float*>(dout.p), nullptr);
    cuda_synchronize();
    const std::vector<float> actual = from_device<float>(dout, 16 * 8);
    ...
    for (int ar = 0; ar < 16; ++ar) {
        for (int br = 0; br < 8; ++br) {
            ...
            for (int d = 0; d < 256; ++d) {
                ...
                const std::uint8_t ac = static_cast<std::uint8_t>(
                    (a[static_cast<std::size_t>(ar * 128 + d / 2)] >> (4 * (d & 1))) & 0x0fU);
                const std::uint8_t bc = static_cast<std::uint8_t>(
                    (b[static_cast<std::size_t>(br * 128 + d / 2)] >> (4 * (d & 1))) & 0x0fU);
                const float av = decode(ac) * scales[static_cast<std::size_t>((ar + group) & 3)];
                const float bv =
                    decode(bc) * scales[static_cast<std::size_t>((2 * br + group) & 3)];
                dot += av * bv;
            }
            expected[static_cast<std::size_t>(ar * 8 + br)] = dot;
        }
    }
    return verify_exact("nvfp4 native mma unit scales", actual, expected);
}
...
std::vector<float> quantize_nvfp4_queries(const std::vector<float>& q, const Geometry& geometry,
                                          std::int32_t tokens) {
    std::vector<float> represented(q.size(), 0.0f);
    std::vector<std::uint8_t> codes(static_cast<std::size_t>(kNvfp4CodeBytes));
    std::vector<std::uint8_t> scales(static_cast<std::size_t>(kNvfp4QuantGroups));
    ...
    for (std::int32_t token = 0; token < tokens; ++token) {
        for (std::int32_t head = 0; head < geometry.q_heads; ++head) {
            ...
            encode_nvfp4_rotated_row(row, 0, codes, 0, scales, 0, decoded, 0);
            for (std::int32_t d = 0; d < kHeadDim; ++d) {
...
HostCache make_cache(const Geometry& geometry, KvCacheStorage storage, std::int32_t max_context,
    ...
    if (storage == KvCacheStorage::K8V4) {
...
std::vector<double> ideal_attention(const std::vector<float>& q, const HostCache& cache,
    ...
    if (cache.storage == KvCacheStorage::Nvfp4Group16 || cache.storage == KvCacheStorage::K8V4) {
        const std::vector<float> q_represented =
            cache.storage == KvCacheStorage::Nvfp4Group16
                ? quantize_nvfp4_queries(q, geometry, tokens)
        ...
        naive_dense_softmax_attention(
            ...
            [&](int d, int head, int position) {
                return static_cast<double>(cache.rotated_k_quantized[cache_index(
                    geometry, cache.logical_capacity, head, position, d)]);
            },
            [&](int d, int head, int position) {
                return static_cast<double>(cache.rotated_v_quantized[cache_index(
                    geometry, cache.logical_capacity, head, position, d)]);
            },
...
public:
    DeviceCache(const HostCache& cache, MappingPattern mapping)
        ...
        if (storage_ == KvCacheStorage::BFloat16) {
            ...
            k_scale_.copy_from_host(ks_physical.data(), ks_physical.size() * sizeof(std::uint16_t));
            v_scale_.copy_from_host(vs_physical.data(), vs_physical.size() * sizeof(std::uint16_t));
        } else if (storage_ == KvCacheStorage::K8V4) {
            const auto k_physical =
                scatter_paged(cache.k_fp8, kHeadDim, geometry_, logical_capacity_,
                              block_table_host_, physical_pages_);
            const auto v_physical =
...
private:
    ...
    void upload_rows(std::span<const HostCache> rows) {
        ...
        if (storage_ == KvCacheStorage::K8V4) {
            std::vector<std::uint8_t> physical_k(k_code_elements_, 0);
            std::vector<std::uint8_t> physical_v(v_code_elements_, 0);
            std::vector<std::uint16_t> physical_ks(k_scale_elements_, 0);
...
int verify_cache(const std::string& label, const HostCache& got, const HostCache& expected,
    ...
    if (expected.storage == KvCacheStorage::BFloat16) {
        ...
        failures += verify_exact((label + " cache-v-scale").c_str(), got.v_scale, expected.v_scale);
    } else if (expected.storage == KvCacheStorage::K8V4) {
        if (verify_private_key_representation) {
            failures += verify_exact((label + " cache-k-code").c_str(), got.k_fp8, expected.k_fp8);
            failures +=
                verify_exact((label + " cache-k-scale").c_str(), got.k_scale, expected.k_scale);
...
ReductionCriterion attention_criterion(KvCacheStorage storage) {
    ...
    if (storage == KvCacheStorage::K8V4) return kAttentionK8V4Criterion;
...
int run_a1_case(const Geometry& geometry, KvCacheStorage storage, const AttentionCase& test_case,
    ...
    failures += verify_cache(label, cache.snapshot(), expected,
                             storage == KvCacheStorage::BFloat16 ||
                                 ...
                                 storage == KvCacheStorage::K8V4);
    if (storage == KvCacheStorage::Nvfp4Group16 || storage == KvCacheStorage::K8V4) {
...
int run_k8v4_cases() {
    ...
    for (const Geometry& geometry : kGeometries) {
        failures +=
            run_a1_case(geometry, KvCacheStorage::K8V4, {1, 0, 1, 801u}, MappingPattern::Identity);
        failures += run_a3_case(geometry, KvCacheStorage::K8V4, {1, 64, 65, 802u},
                                MappingPattern::Fragmented);
        failures += run_a1_case(geometry, KvCacheStorage::K8V4, {6, 61, 192, 803u},
                                MappingPattern::Fragmented);
        failures += run_a1_case(geometry, KvCacheStorage::K8V4, {64, 0, 128, 804u},
                                MappingPattern::Fragmented);
        failures += run_a3_case(geometry, KvCacheStorage::K8V4, {65, 63, 192, 805u},
                                MappingPattern::Offset);
        failures += run_a3_case(geometry, KvCacheStorage::K8V4, {1, 2048, 2049, 806u},
                                MappingPattern::Fragmented);
    }
    failures += run_a1_case(kGeometries[0], KvCacheStorage::K8V4, {1, 64, 65, 807u, false, true},
                            MappingPattern::Fragmented);
    failures += run_a3_case(kGeometries[0], KvCacheStorage::K8V4, {1, 64, 65, 808u, false, true},
                            MappingPattern::Fragmented);
    failures += run_a1_case(kGeometries[1], KvCacheStorage::K8V4, {5, 17, 22, 809u},
                            MappingPattern::Identity);
    failures += run_a1_case(kGeometries[1], KvCacheStorage::K8V4, {7, 17, 24, 810u},
                            MappingPattern::Identity);
    failures += run_a1_case(kGeometries[1], KvCacheStorage::K8V4, {7, 511, 518, 811u},
                            MappingPattern::Fragmented);
    failures += run_a3_case(kGeometries[1], KvCacheStorage::K8V4, {1, 8191, 8192, 812u},
                            MappingPattern::Fragmented);
    failures += run_a3_case(kGeometries[1], KvCacheStorage::K8V4, {1, 16383, 16384, 813u},
                            MappingPattern::Fragmented);
    failures += run_a3_case(kGeometries[1], KvCacheStorage::K8V4, {1, 65535, 65536, 814u},
                            MappingPattern::Fragmented);
    return failures;
}
...
int verify_workspace_capacity_contract() {
    ...
    for (const KvCacheStorage storage :
         {KvCacheStorage::BFloat16, KvCacheStorage::Int8Group64, KvCacheStorage::Fp8E4M3Row256,
          KvCacheStorage::Nvfp4Group16, KvCacheStorage::K8V4}) {
...
int run_softmax_attention_nvfp4_tests() {
    ...
    int failures = verify_nvfp4_mma_probe();
...
int run_softmax_attention_k8v4_tests() {
    ...
    failures += run_quantized_batch_cases(KvCacheStorage::K8V4, 815u);
    failures += report_quantization_quality(KvCacheStorage::K8V4, 819u);
    std::cout << (failures == 0 ? "PASS" : "FAIL")
...
int run_softmax_attention_causal_cache_tests() {
    ...
    int failures = 0;
    failures += verify_nvfp4_mma_probe();
