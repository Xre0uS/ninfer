...
// Group-16 NVFP4 split-KV causal attention for T=1..6. A CTA owns one KV head and all GQA query
// heads, so each persistent K/V byte is streamed once. Native E2M1/UE4M3 QK and FP16 P/V MMA both
// accumulate in FP32. Split merge, normalization, and the inverse D256 rotation remain FP32.
...
__launch_bounds__(WarpsPerCta * 32, MinBlocksPerSm) __global__
    ...
    __shared__ __align__(16) std::uint8_t q_s[Br * CodeRowBytes];
    ...
    __half* v_f16         = reinterpret_cast<__half*>(arena + 2 * Bc * CodeRowBytes);
    __shared__ __align__(16) __half p_s[Br * PStride];
    __shared__ float alpha_s[Br];
    __shared__ __align__(16) std::uint8_t q_scale_s[Br * kKVCacheNvfp4Groups];
...
#pragma unroll
    ...
    for (int index = tid; index < Br * CodeRowBytes; index += Threads) q_s[index] = 0;
    for (int index = tid; index < Br * kKVCacheNvfp4Groups; index += Threads) {
        q_scale_s[index] = 0;
    }
    __syncthreads();
...
#pragma unroll
        ...
        if (lane < kKVCacheNvfp4Groups) {
            ...
            const int physical_byte = causal_small_t_nvfp4_code_swz(row, logical_byte);
            store_vec(q_s + row * CodeRowBytes + physical_byte,
                      make_uint2(quantized.codes_lo, quantized.codes_hi));
            q_scale_s[row * kKVCacheNvfp4Groups + lane] = quantized.scale;
        }
        __syncwarp();
    }
    __syncthreads();
...
#pragma unroll
            for (int kk = 0; kk < QKKs; ++kk) {
                ...
                ldmatrix_x4(
                    af[0], af[1], af[2], af[3],
                    smem_addr(&q_s[(row_base + a_rowoff) * CodeRowBytes +
                                   causal_small_t_nvfp4_code_swz(row_base + a_rowoff, abyte)]));
                const int q_scale_row = row_base + sfa_row;
                const unsigned q_scale4 =
                    load_vec<unsigned>(q_scale_s + q_scale_row * kKVCacheNvfp4Groups + kk * 4);
#pragma unroll
                for (int nt = 0; nt < QKNt; ++nt) {
                    ...
                    ldmatrix_x2(bf[0], bf[1],
                                smem_addr(&k_nvfp4[brow * CodeRowBytes +
                                                   causal_small_t_nvfp4_code_swz(brow, bbyte)]));
                    const int k_scale_row = nt * 8 + sfb_row;
