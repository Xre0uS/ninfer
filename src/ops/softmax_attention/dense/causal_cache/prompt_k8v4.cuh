...
__device__ __forceinline__ void causal_prompt_k8v4_store_swz(std::uint8_t* tile, int row, int d,
                                                             std::uint8_t code) {
    const int col_b16 = d >> 1;
    const int byte    = d & 1;
    const int off     = (row * kCausalPromptK8V4DB16 + causal_prompt_swz(row, col_b16)) * 2 + byte;
    tile[off]         = code;
}

__device__ __forceinline__ int causal_prompt_k8v4_p_swz(int row, int col) {
    if constexpr (kCausalPromptK8V4Bc == 32) { return (((col >> 3) ^ (row & 3)) << 3) | (col & 7); }
...
#pragma unroll
        for (int r = 0; r < 8; ++r) {
            const int d = lane + 32 * r;
            causal_prompt_k8v4_store_swz(q_fp8, row, d, kv_cache_fp8_quant_code(values[r], inv));
        }
...
#pragma unroll
            for (int nt = 0; nt < QKNt; ++nt) {
                ...
                bl0 += p00 + p01;
                bl1 += p10 + p11;
                p_s[row0 * Bc + causal_prompt_k8v4_p_swz(row0, col0)] = __float2half_rn(p00);
                p_s[row0 * Bc + causal_prompt_k8v4_p_swz(row0, col1)] = __float2half_rn(p01);
                p_s[row1 * Bc + causal_prompt_k8v4_p_swz(row1, col0)] = __float2half_rn(p10);
                p_s[row1 * Bc + causal_prompt_k8v4_p_swz(row1, col1)] = __float2half_rn(p11);
            }
        ...
        for (int k = 0; k < PVKs; ++k) {
            ...
            ldmatrix_x4(pf[0], pf[1], pf[2], pf[3],
                        smem_addr(&p_s[(row_base + a_rowoff) * Bc +
                                       causal_prompt_k8v4_p_swz(row_base + a_rowoff, pcol)]));
#pragma unroll
            for (int n = 0; n < PVNtPerWarp; ++n) {
